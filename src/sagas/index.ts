import { Context, interrupt, manualToggleDebugger, resetDebugger, resume, runInContext } from 'js-slang'
...
function* workspaceSaga(): SagaIterator {
  ...
  yield takeEvery(actionTypes.DEBUG_RESET, function*(action) {
    ...
    lastDebuggerResult = yield undefined 
  })
...
function* evalCode(code: string, context: Context, location: WorkspaceLocation, actionType: string) {
  ...
  const { result, interrupted, paused } = yield race({
    result: call(runInContext, code, context, { scheduler: 'preemptive', debugger: debug}),
    /**
